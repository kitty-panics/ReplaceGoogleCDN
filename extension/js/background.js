...
let use_nginx_proxy = (details, proxy_provider) => {
    ...
    // 中文域名编码转换 punycode 标准编码: punycode('点') = 'xn--3px'
    // 替换点. 为了正则表达式好区分 _xn--3px_ 仅仅是分隔符号，可以自己定义分隔符号
...
chrome.webRequest.onBeforeRequest.addListener(
    function (details) {
        ...
        // 方法二： 使用 NginX 架设的服务动态地址替换
        // 测试例子：打开 https://ww.google.com (仅用于学习技术)
        // https://2_www_xn--3px_google_xn--3px_com.proxy.xiaoshuogeng.com

        // return {
        //     redirectUrl: use_nginx_proxy(details,suffix_domain)
        // };

        let url = details.url.replace("http://",          "https://");
        /* 中科大 */
        //url = url.replace("ajax.googleapis.com",          "ajax.proxy.ustclug.org");    // Google 前端库
        ...
        /* 极客族 */
        //url = url.replace("ajax.googleapis.com",          "gapis.geekzu.org/ajax");     // Google 前端库
